...
class AuthController{
	async refresh(req, res){
        ...
        if(info.isAuth){
            res.cookie('token', info.tokens.refreshToken, {maxAge: 30 * 24 * 60 * 60 * 1000, httpOnly: false});
    ...
    async login(req, res){
        ...
        try {
            ...
            res.cookie('token', tokens.refreshToken, {maxAge: 30 * 24 * 60 * 60 * 1000, httpOnly: false});
            return res.status(200).json({
